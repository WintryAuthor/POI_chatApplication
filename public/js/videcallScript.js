...
let toogleMic = async () => {
    ...
    if (audioTrack.enabled) {
        ...
        document.getElementById('mute-btn').style.backgroundColor = 'rgb(255, 80, 80)'
    } else {
        audioTrack.enabled = true;
        document.getElementById('mute-btn').style.backgroundColor = 'rgb(179, 102, 249, .9)'
    }
}

window.addEventListener('beforeunload', leaveChannel);
document.getElementById('camera-btn').addEventListener('click', toogleCamera);
document.getElementById('mute-btn').addEventListener('click', toogleMic);
